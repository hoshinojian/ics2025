...
//instpat_match完成从指令中取出来寄存器、立即数和执行
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  //src1就是R[rs1]
  INSTPAT("??????? ????? ????? 000 ????? 0100011", sb, S, Mw(src1 + imm, 1, src2));//sw rs2, offset(rs1) [addr] = data, [第一个] = 第三个
  ...
  INSTPAT("????????????  ????? 000 ????? 1100111", jarl, I, {
    ...
    if (rd == 1) {
        log_ftrace(type_jalr_call, s->pc, target_);
    }
    // 2. 如果 rd == 0 (不保存) 且 rs1 == 1 (源地址来自 ra)，这是函数返回 (Ret)
...
/* RISCV M EXTENSION        */
  ...
  INSTPAT("??????? ????? ????? 001 ????? 1110011", csrrw, I, {R(rd) = 0;});// rs1写入csr, csr原先的值写入rd
